...
!function ($) {
    ...
    var _bootstrap_table_GlobalString = { "bootstrap_table_NoResult": "没有匹配的结果" };
    ...
    var BootstrapTable = function (el, options) {
        ...
        //获取本地化字符串
        $.get(_PORTALROOT_GLOBAL + "/Ajax/GlobalHandler.ashx", { "Code": "bootstrap_table_Loading,bootstrap_table_NoResult,bootstrap_table_Search" }, function (data) {
            if (data.IsSuccess) {
                _bootstrap_table_GlobalString.bootstrap_table_NoResult = data.TextObj.bootstrap_table_NoResult;
                $(".bootstrap_table_Loading").text(data.TextObj.bootstrap_table_Loading);
                $(".bootstrap_table_NoResult").text(data.TextObj.bootstrap_table_NoResult);
                $("#bootstrap_table_Search").attr("placeholder", data.TextObj.bootstrap_table_Search);
                //不支持placeholder的浏览器，jquery.placeholder会把placeholder赋值给value以模拟效果
                if ($("#bootstrap_table_Search")[0].value != "") {
                    $("#bootstrap_table_Search")[0].value = data.TextObj.bootstrap_table_Search;
                }
            }
        }, "json");
        ...
        //$.ajax({
        //    type: "GET",
        //    url: _PORTALROOT_GLOBAL + "/Ajax/GlobalHandler.ashx",
        //    dataType: "json",
        //    async: false,
        //    cache: false,
        //    data: "Code=bootstrap_table_Search",
    ...
    BootstrapTable.prototype.initContainer = function () {
        this.$container = $([
            '<div class="bootstrap-table">',
                '<div class="fixed-table-toolbar" style="height:30px;"></div>',
        ...
        if (this.options.search) {
            ...
            html.push(
                '<div class="pull-right search">',
                    '<input class="form-control" type="text" id="bootstrap_table_Search" placeholder="搜索...">',
                '</div>');
    ...
    BootstrapTable.prototype.initTreeBodyRow = function (that, html, item, i, parentpath) {
        ...
        html.push('<tr',
            ...
            haschildren ? sprintf(' data-childrenkey="%s"', parentpath + i + "_") : "",
            '>'
        );
